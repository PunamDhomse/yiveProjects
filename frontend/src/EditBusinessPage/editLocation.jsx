...
export default class EditLocation extends Component {
    ...
    componentWillMount() {
        this.props.returnCountries().then(countries => {
            ...
            let country = this.props.val.country;
            if (!this.props.val.country) {
                country = countries.data[0]._id;
            }
            this.props.returnRegions(country).then(regions => {
                this.setState({ regions: regions.data[0].regions });
                this.setState({ activeCountry: this.props.val.country, activeRegion: this.props.val.state });
            });
        });
    }
            ...
            <div className="row">
                {val.status ? <div className="col-12 addLocation my-3 p-3 bg-light rounded">
                    ...
                    <Form.Group >
                        ...
                        <Form.Control as="textarea" rows="2" data-fieldname="address" name={addressId} id={addressId} data-id={idx} placeholder="Enter location address" value={businessLocation[idx].address} onChange={this.props.handleChange} />
                    </Form.Group>
                    <Form.Group >
                        ...
                        <Form.Control as="select" data-fieldname="country" name={countryId} id={countryId} data-id={idx} as="select" onChange={this.countryChange} >
                            ...
                            {this.state.countries &&
                                this.state.countries.map((country, index) =>
                                    <option selected={this.state.activeCountry === country._id} value={country._id}>{country.name}</option>
                                )};
                                              </Form.Control>
                    </Form.Group>
                    <Form.Group >
                        ...
                        <Form.Control as="select" name={stateId} data-fieldname="state" id={stateId} data-id={idx} as="select" onChange={this.props.handleChange} >
                            ...
                            {this.state.regions &&
                                this.state.regions.map((region, index) =>
                                    <option selected={this.state.activeRegion === region._id} value={region._id}>{region.name}</option>
                                )};
                                              </Form.Control>
                    </Form.Group>
                    <Form.Group>
